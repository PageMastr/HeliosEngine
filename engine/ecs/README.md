...
| `entity_id.h` | Block ids 41/5/17 (05 §1.4.5, same layout and golden vectors as Go `pkg/idgen`): `EntityIdMinter` (lock-free minting from held blocks, refill at half use, 1 h retirement, async delivery), `IdBlockSource` / `LocalIdBlockSource` (the AllocateIdBlocks rule in-process), `ClientLocalIdAllocator` |
| `registry.h` | `U64Map` (open addressing, backward-shift delete), `NetHandleTable` (content-reserved slots, FIFO reuse with delay, generations), `EntityRegistry` (EntityId ↔ Entity ↔ NetHandle) |
...
| `command_buffer.h` | `CommandBuffer` (spawn → `TempEntity`, destroy, add/remove/set, setParent, reparent (InFrame), dock/undock), `SpawnDesc`, `EntityRef` |
...
not raw `ecs_delete`. Children instantiated from prefab hierarchies get identities too.
...
still minted in command order). Fused initial values do not fire flecs OnSet observers.
...
`ecs_tests` (doctest, 89 cases) covers: block-id layout against the shared Go golden vectors,
...
with label `bench`. Results and the RT-01 verdict are in SPIKES.md §3.

## Known limitations

* RT-01 fails its "9k structural ops ≤ 1.5 ms" criterion. The warm burst takes about 5–6.7 ms on the
  dev VM, while the same operations on the raw flecs C API take about 1–1.7 ms. The difference is
  World bookkeeping. All other RT-01 criteria pass. SPIKES.md §3.3–3.4 has the breakdown and the
  optimization list for the ADR-004 decision.
...
* Fused spawn values bypass flecs OnSet observers; command-buffer spawns of the same signature are
