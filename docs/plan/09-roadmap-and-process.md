...
|---|---|---|---|
| Region leases in NATS KV with a 3 s TTL and holder self-fencing | 05 §1.4.2 | **Reworked.** Generations are allocated in PG (`region_lease.lease_gen` since WP-0.15r, `placement_log`). The 12 s liveness TTL is only the orchestrator's fallback. `orchestrator.Agent` implements the holder rule, and `service_test.go` asserts that no `LEASES` bucket exists. CONF-03's `conformance/holder_rule` is `TestConformance/holder_rule` (WP-0.15r): 60 s without a control plane (no responders, timeouts, `unavailable`), then only the region whose generation rose is dropped | CONF-01…03 in CI |
...
| Snowflake node-ID leases | ADR-004; 05 §1.4.5 | **Reworked.** `id_alloc` blocks, `pkg/idgen.Minter` and the shared vectors `testdata/vectors/block_ids.json`. The last stale `-- Snowflake` comment, on `account.account_id`, cites 05 §1.4.5 (WP-0.15r) | — |
| Schema names `identity` and `orchestrator` | 05 §1.4 (**Owns**), §3 | **Reworked** (WP-0.15r). `svc_identity` and `svc_orch`. `migrations.Up` renames a pre-rework database's schemas in place after their last legacy file, so applied files keep their statements and a fresh database runs the same files. Integration tests check the net schema, that upgrade, and concurrent first starts | CONF-06 in CI. `identity/00001` and `orchestrator/00001`–`00002` keep the legacy names and `migrations.go` names them as strings, so WP-0.2's lint evaluates the net schema or takes reviewer-approved `conformance:allow` lines |
| Plain-text `email` and `email_norm` columns | 05 §3, §6.6 (Phase 0: per-account DEKs) | **Reworked** (WP-0.15r). `email_ct` (XChaCha20-Poly1305 under the account's DEK, AAD = table, column, account ID), `email_bidx` = HMAC-SHA256(pepper, lower(email)), and `svc_identity.subject_key` with the DEK wrapped by the KEK (`pkg/pii`; dev key files). Login looks up the blind index. Expand (00002), a Go step that encrypts stored addresses and nulls the plain-text copies (3), contract (00004), and a table rewrite so no old row version keeps them (00005). Expand and contract land in one WP, a Phase 0 deviation from 05 §3.3 recorded in `services/README.md` (no release had shipped) | CONF-07 in CI. `identity/00001` keeps `email` and `email_norm`, so the lint evaluates the net schema here too |
| Plain-text `client_ip` in `refresh_token` and `audit_log` | 05 §1.17, §6.6 | **Reworked** (WP-0.15r). `refresh_token.client_ip_ct` is sealed under the account's DEK with the token row in its AAD. `audit_log` rows hold only pseudonymous IDs: IPs go to `svc_identity.login_history`, sealed under the account's DEK, and an unknown login, which has no subject, keeps no IP. After 05 §6.6's 90 days an hourly job deletes the history and clears the tokens' IPs. Migration 3 verifies the old hash chain row by row and against `audit_head`, then re-chains it without IPs (row format 2, which also covers 05 §1.17's `note_digest`) and records the old head in a marker row and the log | CONF-07 in CI. The one-time re-chain is a Phase 0 deviation from 05 §1.17's append-only log, recorded in `services/README.md`; WP-1.14's first anchor covers the marker row |
| GM free text in `account.ban_reason` and the ban's `audit_log.detail` | 05 §1.17, §6.6 | **Reworked** (WP-0.15r). The reason is `account.ban_reason_ct`, sealed under the account's DEK (AAD = table, column, account ID), and the ban's chained detail keeps only `until`. Migration 3 seals stored reasons and strips `reason` from ban rows in the same re-chain | — (WP-1.14's GM audit adds `audit_note` for other GM text) |
| `zone.lease_gen` instead of `region_lease` | 05 §1.4.2 | **Reworked** (WP-0.15r). `svc_orch.region_lease` has one row per zone region (v0: the `Whole` region, `region_id` = zone ID). Placement bumps `lease_gen` there, term-fenced, and `zone` keeps no lease. The migration carries each zone's generation over | Rows per `ZonePartitionDef` region arrive with multi-region zones, and a region field in assignments with them |
| `RegisterProcess` has no failure domain; heartbeats omit held regions | 05 §1.4 API | **Reworked** (WP-0.15r). `fd{az, rack, host}` and `serverBuild` are stored in `svc_orch.process`; heartbeats carry `held[{region, leaseGen}]`, which the leader records per process. Both are optional on the wire, so the C++ client still matches the Go contract vectors | Open for WP-0.14, whose C++ `OrchestratorClient` must send them (Director to record under D3); Phase 2's failure detection uses them |
...
| `engine/script` | WP-0.10 | In order | 8 (with WP-0.10r) | 02 §7.4 (R5-02.4) | **Conforming with WP-0.10r** (in review). `create()` refuses native codegen on cells and world-script hosts, and the `codegen-fornloop-fuel` and `fuel-counter` patches are in `third_party/luau/patches/`, listed in `MANIFEST.md` and checked by `lint_vendor_patches` |
...
| `services/` (`cmd`, `internal`, `pkg`, `migrations`) | WP-0.15, 0.15r | Ahead (draft v1) | 6 (with WP-0.15r) | 05 §6.5 (Phase 2–3 scope); 05 §3.6's progression deltas and §1.4.6's replicant placement (not built) | **Reworked by WP-0.15r** (in review). No round-5 delta. The open rows of (a) that were re-checked on 2026-09-25 (the `identity` and `orchestrator` schemas, plain `email_norm`, no `region_lease`) are reworked, and so are the plain-text client IPs and ban reasons ((a)) |
...
| K39 | Luau fuel metering misses RT-13: native codegen counts different fuel from the interpreter, and the metering hook costs more than its budget | Tech | **Fired 2026-09-25**; H/M. Both mitigations in review (WP-0.10r) | **What fired:** both clauses, measured in WP-0.10 on Luau 0.739. Codegen charges +1 fuel per numeric `for` left by `break` or `return`, because the loop interrupt sits at the top of the body instead of in `FORNLOOP`. That breaks the fuel identity 04 §10.2's replay needs. The `interrupt` hook costs ≈ 12–15 % against ≤ 10 %. **Mitigations (WP-0.10r):** the vendored `third_party/luau/patches/codegen-fornloop-fuel` (the interrupt in `FORNLOOP`, `CodeGen/src/IrTranslation.cpp`), with cells and world-script hosts interpreter-only (`VmConfig` refuses codegen there; the patch is the precondition for lifting that, which is 02 §8.1's P3 "codegen opt-in on cells" item), and the vendored `fuel-counter` inline-counter VM patch. Both are in `sim_abi.script` and are rebased at every Luau bump (K10) | Runtime lead | Fired: RT-13's fuel-identity and overhead clauses red. Next: either patch unmerged at the Phase 0 exit; overhead > 10 % with `fuel-counter`; a Luau bump the patches do not rebase onto within the quarterly window |
...
| WP-0.3 nightly and scorecard (`scorecard.jsonc`, `tools/scorecard`, `tools/status`, `tools/milestone`, `.github/workflows/nightly.yml`) | **In progress** (PR 3 of 3; part 1 merged as #10, part 2 is #11) | `scorecard.jsonc` registers the 18 Phase 0 criteria (6 measured, 9 partial, 3 unmeasured; every gap names its owner) and 14 exit items; each threshold quotes the plan. CTest `lint_scorecard` checks it against the plan's criterion tables and 09 §2.1's exit, `ci.yml`, `nightly.yml` and each native build's CTest and doctest inventory. The nightly evaluates every criterion from CTest JUnit, doctest XML and `go test -json` on Linux (GCC, Clang, ASan) and Windows (VS 2026, VS 2022, clang-cl), with 3-night streaks (§5.6), the perf history and its ±5/±10 % comparator (§5.8), `net_bench --gate` and NS-0.4's 1 h libFuzzer run per target. `tools/milestone/validate.ps1` (§5.9) and `tools/status/snapshot.py`, which absorbs `check_status.cmake`, are in. *Not yet run:* the nightly has no run history on GitHub, and `validate.ps1` has not run on Windows. **Next:** PLAN.md §11 and the §5.8 ratchet state, which D6 also names, generated by `snapshot.py` |
...
| WP-0.10 Luau host v0 and WP-0.10r fuel metering (`engine/script`, `third_party/luau/patches/`) | WP-0.10 **done** (committed; 79 tests) except two RT-13 clauses. WP-0.10r closes them: **in progress** (in review, not merged; 97 tests). K39 has fired | **Fuel identity:** the vendored `codegen-fornloop-fuel` patch emits the numeric-`for` interrupt in `FORNLOOP`, so native code counts the interpreter's fuel (the pinned test passes, and fails with the patch reverted: +80 fuel). `create()` refuses native codegen on cells and world-script hosts, as 02 §7.4 and 04 §10.2 now say (WP-0.10r amended them). **Metering overhead:** the vendored `fuel-counter` inline counter; on the perf workload, with the clock read every 64 fuel as on cells, the host costs ≈ 2–8 % against unmetered plain Luau (≈ 25–28 % before in the same run; WP-0.10 measured 12–17 % without clock reads), and `perf: fuel metering overhead …` asserts ≤ 10 % in optimized builds. `lint_vendor_patches` checks that every vendored patch is applied and listed in `MANIFEST.md`. The DAP adapter is WP-1.6 |
...
| WP-0.15 Go backend skeleton, with WP-0.15r (`services/`: `cmd/helios-backend`; `internal/app`, `backend`, `identity`, `integration`, `orchestrator`, `platform`, `session`, `stack`; `pkg/authn`, `clock`, `connecttoken`, `idgen`, `keyring`, `pii`, `ratelimit`, `rpc`, `testkit`) | **Done** (committed; 161 tests; Go 1.27.1). WP-0.15r **Reworked** the open rows of §5.10.4 (a) | `helios-backend` runs on embedded PostgreSQL 18, NATS and miniredis. It has identity, netcode connect tokens byte-exact with the C implementation, and the orchestrator with PG leadership and ID blocks. WP-0.15r added the `svc_identity` and `svc_orch` schemas, e-mail as `email_ct` and `email_bidx`, client IPs as `client_ip_ct` and `login_history`, and ban reasons as `ban_reason_ct` under per-account DEKs, audit rows without IPs or GM text, `region_lease`, `fd` and `serverBuild` in registration, held regions in heartbeats, and `conformance/holder_rule`. CONF-01…09 wait for WP-0.2's lint. D4's hold on further backend WPs (0.16, 1.13, 1.14) ends when WP-0.15r merges |
