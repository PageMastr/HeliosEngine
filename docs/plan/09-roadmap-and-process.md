...
scope unless their Rework cell names another WP.
...
| Plain-text `client_ip` in `refresh_token` and `audit_log` | 05 §1.17, §6.6 | **Reworked** (WP-0.15r). `refresh_token.client_ip_ct` is sealed under the account's DEK with the token row in its AAD. `audit_log` rows hold only pseudonymous IDs: IPs go to `svc_identity.login_history`, sealed under the account's DEK and purged after 05 §6.6's 90 days, and an unknown login, which has no subject, keeps no IP. Migration 3 verifies the old hash chain, then re-chains it without IPs (row format 2, which also covers 05 §1.17's `note_digest`) and records the old head in a marker row | CONF-07 in CI |
| GM free text in `account.ban_reason` and the ban's `audit_log.detail` | 05 §1.17, §6.6 | **Open.** The dev `Ban` RPC stores its reason in plain text | WP-1.14 (GM audit): the reason goes to `audit_note`, sealed under the subject's key, and the chain covers its `note_digest` |
