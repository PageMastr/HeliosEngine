...
- A WP-class PR must land as exactly one commit, with the same final tree as the reviewed PR head.
  A tree mismatch means the landed tree was not the one checked at integration verify (§5.2 step 6).
...
| WP-0.1 CI matrix | **Partial** (committed) | **PR:** Windows MSVC primary (`windows-latest`, VS 2026, MSVC 14.51), `windows-msvc-floor` (`windows-2022`, MSVC 14.44), Windows clang-cl, Linux GCC, Linux Clang (lavapipe GPU tests), Linux headless, the MinGW cross-build, Go on Windows and Linux, and Go integration (embedded PostgreSQL, non-root). **Nightly:** ASan and the VS 2026 and VS 2022 MSBuild builds. **SARIF:** the Linux headless lint run publishes an artifact and, for same-repo PRs and `main`, code scanning results. **Post-merge:** an independent `merge-policy` workflow is triggered by pushes to `main` and compares each landed range with its merged PR (§5.2a). Its REST commit-list limit is 250 original PR commits, with larger PRs reported as unverified. GitHub skip markers can suppress push workflows until the queue and `main` ruleset reject them. The future merge queue must push with an Integrator GitHub App or PAT token: pushes made with a workflow's `GITHUB_TOKEN` do not trigger this audit. Track REST API version `2022-11-28` for migration because the audit uses `merge_commit_sha`. **Missing:** scorecard and nightly perf (WP-0.3), plus the libFuzzer nightly (NS-0.4). The modular `windows-msvc-dev` job waits for WP-0.6c. The merge-queue script and `main` ruleset are still absent; `CMakePresets.json` declares `cmakeMinimumRequired` 3.28, matching the top-level minimum |
...
| 4 | 1 | **WP-0.1** (rest): the merge-queue script and the `main` ruleset | AAA-PLT-1. The queue turns "committed" into "merged" (§5.2a) |
