-- WP-0.15r, contract step (05 §3.3): after migration 3 encrypted every stored address, client IP
-- and ban reason and nulled the plain-text copies, the plain-text columns go (05 §1.17, §6.6;
-- CONF-07). 00005 then rewrites the tables so no dead tuple keeps them. Plain text written
-- before the upgrade can still be in WAL and in base backups taken before it.
--
...
ALTER TABLE svc_identity.account
    ...
    DROP COLUMN ban_reason;   -- GM free text: now ban_reason_ct
