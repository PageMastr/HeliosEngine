...
| WP-0.8 ECS, records, assets v0 (`engine/ecs`) | **Partial.** The ECS on flecs is **done** (committed; 90 tests). Records (`.hrdb`), `.meta`, the DDC and `.hpak` v0 are **not started** | **The RT-01 pre-bench fails one clause, and K2 has fired.** Every criterion passes except 9k structural ops, at 3.4–6.7 ms against ≤ 1.5 ms. Raw flecs does the same ops in ≈ 0.9 ms, so the overhead is the Helios World wrapper: identity maps, the structural log, command fusion and `componentInfo` lookups. **ADR-004a is open.** The wrapper is optimized first (WP-1.1a) and flecs is kept; a custom ECS comes only if the optimized wrapper still fails RT-01 at the Phase 1 gate. *WP-1.1a (in progress, not merged):* on the dev VM the burst takes 1.26–1.27 ms per configuration with GCC and 0.96–0.98 ms with Clang (2.9–3.3 ms before), and ADR-004a's indicator M1 is 1.54× (GCC) and 1.59× (Clang) against ≤ 1.6× (medians of 9 runs; single runs up to 1.73× and 1.80×; 1.24–1.30× in instructions), so **M1 is met on the median, without much margin**. `InFrame` stays fragmenting (M5). ADR-004a §7, `engine/ecs/SPIKES.md` §5 |
...
| Fired risks and failing gates | **K2** (RT-01 structural ops; ADR-004a, WP-1.1a: M1 at 1.54–1.59×, met on the median of runs without much margin), **K39** (RT-13 fuel identity and overhead; WP-0.10r) and **K5b** (armed by WP-0.9c's red outcome; `pcg_tests_perf` fails by design) | §7. K3's hazard was found and mitigated in spike (a) |
