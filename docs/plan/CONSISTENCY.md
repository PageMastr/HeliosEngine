...
| 9 | 09 §5.2a | #7, WP-0.1 | WP-class squashes must match the reviewed head tree. A mismatched tree is reverted and resubmitted through a new PR after integration verification. Both violation and unverified results fail the post-merge check; exit 2 is rerun, then persistent non-association is treated as a direct push and other persistent verification failures are checked by hand against the reviewed head and recorded on the PR | None. The checker and its workflow arrive in this PR and implement the rule | `test_wp_squash_with_changed_tree_fails`, `test_main_exit_status_separates_violation_and_unverified`, and `test_incomplete_pr_commit_metadata_is_unverified` (`tools/ci/test_merge_policy.py`) |
