...
| Not started | — | WP-0.3 (nightly, scorecard, `validate.ps1`); WP-0.4 (the user's `win-gpu` runner, a human action); WP-0.6c; WP-0.9 (physics, PCG); WP-0.16 (patching); WP-0.17 (launcher, client); WP-0.18 (ToolsFramework, editor); WP-0.20 (*Cinder Reach* `content/`). Also WP-0.7b and the rest of WP-0.8 |
| Fired risks and failing gates | **K2** (RT-01 structural ops; ADR-004a, WP-1.1a) and **K39** (RT-13 fuel identity and overhead; WP-0.10r) | §7. K3's hazard was found and mitigated in spike (a) |
...
| 4 | 1 | **WP-0.1** (rest): SARIF, the merge-queue script and `merge-policy`, the `main` ruleset, the presets' CMake minimum | AAA-PLT-1. The queue turns "committed" into "merged" (§5.2a) |
