...
| WP-0.3 nightly and scorecard (`scorecard.jsonc`, `tools/scorecard`, `tools/status`, `.github/workflows/nightly.yml`) | **In progress** (PR 2 of 3) | `scorecard.jsonc` registers the 18 Phase 0 criteria (7 measured, 7 partial, 4 unmeasured; every gap names its owner) and 12 exit items. CTest `lint_scorecard` checks it against the plan's criterion tables and 09 §2.1's exit, `ci.yml`, `nightly.yml` and each native build's CTest and doctest inventory. The nightly evaluates every criterion from CTest JUnit, doctest XML and `go test -json` on Linux (GCC, Clang, ASan) and Windows (VS 2026, VS 2022, clang-cl), with 3-night streaks (§5.6), the perf history and its ±5/±10 % comparator (§5.8), `net_bench --gate` and NS-0.4's 1 h libFuzzer run per target. `tools/status/snapshot.py` absorbs `check_status.cmake`. *Not yet run on GitHub:* the nightly has no run history. **Next (PR 3):** `tools/milestone/validate.ps1` |
