...
> world-script editor integration and ED-23 in WP-3.12). **Conforms to:** ADR-001…016 and the phase vocabulary in `README.md`.
...
the WP-0.4 runner policy check green; the conformance lint clean on the full tree with WP-0.15r, WP-0.2r and
WP-0.5r merged (§5.10);
...
| WP-1.5 | CR | Physics, anim, nav, audio | 02 §7.1–7.3 | 1.2 | Shared `CharacterVirtual` mover, hulls, ozz graphs + hitbox poses through `det::HitboxSampler` (no estimate intrinsics; 04 §10.2), Recast per grid with `qsort` call sites patched to `det::sort`, audio events | RT-03 (full) |
...
| WP-2.4 | NS | 500 per zone | 04 §2.5, §5.6–5.7, §7–8 | 1.11, 1.12 | AIMD, deltas, LOD groups, lag comp + projectiles, predicted abilities, TiDi, degrade stage, multi-instance cells, overflow layers, trunk sharding with the fixed trunk IO pool and per-trunk buffer sizing (04 §2.6), `SimInbox` + replay, `det::sort` and the asserted MXCSR policy (04 §10.2) | NS-2.1–2.6; AAA-SRV (Ph2), SEC-3 |
...
| WP-3.11 | IN | Security Ph3 | 04 §9 | 3.1 | 24 CPU-h fuzzing, text sanitization, rate limits, rewind caps, X25519 re-key design; focused external test of HTP, auth and the patcher before the public beta (§4.3 H7) | AAA-SEC-5, SEC-7 |
...
BE-A11, A13, A15, A20; GP-3 (Ph3), 4c, 4d (Ph3 layouts), 5, 7, 8, 10 (Ph3), 11 (Ph3), 13, 14 (Ph3), 15 (Ph3), 16, 17 (Ph3); ED-9…12, 14, 15 (Ph3), 19…21, 23; CL-7, 13, 16 (Ph3), 23 (Ph3), 24 (Ph3); funding gate F3 (§4.3.6).
...
(Ph4), 6; SEC-8; SRV-9 (Ph4, ≤ 1 s in persistent world zones); TOOL-4, 8, 9, 10; ITR-8; PLT-5; RT-10, 12, 15, 22; RC-8…11; NS-4.1–4.7; BE-A5 (10k), A8, A9, A14 (Ph4), A16, A18, A19; GP-3, 11 (Ph4), 14 (Ph4), 15 (Ph4), 17 (e);
...
- **D7 Working-tree code and review rounds.**
  ...
  - *Recording.* The module README of such code records `Plan-Rev: <n>`, the plan revision it was written
    to, so D3 can find it before it has a commit trailer.
...
| Windows failure path and gate-object rules | 02 §1.1 | **Non-conforming.** The hook ends with `ExitProcess`, which would run mimalloc's `.CRT$XLY` detach hook once the gate sits in a TLS callback. It shows a message box whenever stderr is unusable, not by the PE subsystem, and has no `HELIOS_CPU_GATE_SILENT`. The objects keep `/GS` (the allowlist admits `__security_check_cookie`). The exports are `helios_cpu_gate_run` and `helios_cpu_gate_crt_entry`, and there is no `helios_cpu_gate_verdict` or `platformInit` check | WP-0.5r: `TerminateProcess(…, 78)`; the subsystem test and the silent switch; `/GS-` and `-fno-stack-protector`; the three exports; the verdict check (CONF-12; audit check 2) |
...
| Editor UI (07 §4.4) | `helios-uitest`: every tool opened by injected input and layout-linted at 5 scales (PR); ꟻLIP goldens of every tool at 100/200 % in dark and high-contrast themes, dual-path scenario hashes, UI-level replays of ED-2, 7, 10, 16, 17 and 23 (nightly; Xvfb + lavapipe on Linux, `win-gpu` post-merge on Windows); multi-cell PIE seam scenario (nightly, headless) | PR, nightly | ED-14, ED-15, ED-23 |
...
The Director refreshes this table at the start of every round (§5.10.2 D6). From WP-0.3 on,
`tools/status/snapshot` generates it. "Working tree" means code that exists but has not yet passed §5.2's
review and merge queue as its WP.

| Item | Status | Evidence / gap |
|---|---|---|
| Plan: ADR 00, sections 01–09, `docs/PLAN.md` | Draft v5 (round-4 fixes) | Integration notes reconciled in §3.2; cross-section fixes logged in `CONSISTENCY.md`. Round 4 found the ISA drift below, and D7 now covers working-tree code (§5.10.2) |
| CMake presets (MSVC, VS 2022, VS 2026, clang-cl, GCC, Clang, ASan, headless, MinGW) | Done | The top-level `CMakeLists.txt` requires 3.28, but `CMakePresets.json` still declares `cmakeMinimumRequired` 3.24 (WP-0.1). The IDE presets become modular in WP-0.6c (ADR-001a rule 5) |
| 25 vendored dependencies with `MANIFEST.md` | Done | RmlUi, text stack, nats.c, sentry, Slang, libopus due by phase. **Gap:** `third_party/CMakeLists.txt` still builds SDL3 with `SDL_RENDER OFF` and `SDL_WAYLAND OFF`, which the launcher cannot use (reconciliation #19; WP-0.17) |
| CI (`.github/workflows/ci.yml`, `nightly.yml`) | Partial | **PR:** `windows-msvc` (VS 2026, the primary), `windows-msvc-floor` (`windows-2022`, `msvc-dev-cmd` toolset 14.44), `windows-clang-cl`, `linux` (GCC and Clang, lavapipe under Xvfb), `linux-headless`, `mingw-cross`, Go `services` on Windows and Linux (version read from `go.mod`), and `services-integration` (embedded PostgreSQL and NATS, netcode C interop). **Nightly:** Linux ASan/UBSan and the `windows-vs2026` and `windows-vs2022` MSBuild builds. **Missing for AAA-PLT-1:** `windows-msvc-dev` (modular; waits for WP-0.6c), ccache, SARIF, the `main` ruleset, the merge queue script and `merge-policy` (§5.2a), and scorecard reporting (WP-0.3) |
| `engine/core` | Mostly done; the CPU gate is non-conforming | 174/174 test cases pass (headless GCC 13.3 build of the working tree on 2026-09-25, 0 warnings), including 11 CPU-gate cases. CPUID (`core/cpu.h`), process spawn (`process.h`), async reads (`async_io.h`) and the Windows exe manifest (`engine/platform/win/helios.manifest`, checked by `lint_windows_manifest`) are in the working tree for WP-0.5. The gate probe and hooks are in the working tree but built for the retired model, with the Windows entry in `.CRT$XIB` (WP-0.5r, §5.10.4) |
| `engine/math` | Done in the working tree | 115/115 test cases pass (same build). `det::exp`, `ln`, `pow` and `asinh` (`src/det_exp.cpp`), and `Q16`, `Q32`, `Fixed64` and integer `rsqrt` (`fixed.h`), are in the working tree for WP-0.5 |
| LAYER/EDITOR_ONLY checks; module order; licence, IP-name and manifest lints | In the working tree (WP-0.2) | `cmake/HeliosLayering.cmake` and `HeliosModule.cmake` provide `helios_declare_module` with `LAYER`, `HEADLESS`, `EDITOR_ONLY` and `PEERS`, the `HELIOS_MODULE_ORDER` check and executable roles. The 14 layering fixtures, the licence, IP-name and manifest lints and their fixtures pass: 39/39 `lint` tests in the same build. The conformance lint (`tools/conformance`) does not exist yet |
| ISA levels and CPU gate (02 §1.1, ADR-011 amendment) | **Partial and non-conforming** (working tree, written to the retired per-file design); rework WP-0.2r and WP-0.5r | Present: `cmake/HeliosIsa.cmake` (the `avx2` and `base` flag sets as functions, `helios_avx2_sources()`, `helios_cpu_gate()`); `cmake/isa_allowlist.cmake` (per-target `tp_jolt` and per-file `*_avx2` AVX2 lists, plus the gate and self-dispatch lists that 02 keeps); `tools/lint/isa_audit.cmake` (checks 1–3 against the allowlist, ELF only for check 3) and `tools/ci/msvc_gate_audit.ps1` (check 2 on Windows, not yet called by `ci.yml`); the gate with its hooks (`.preinit_array` on Linux, which conforms; `.CRT$XIB` on Windows, which does not). Missing: image levels, `<module>@base` builds, the `base`-links-`avx2` configure error, checks 3 (Windows and `.dynsym`), 4 and 5, and gate linking into `helios_runtime`. `third_party/CMakeLists.txt` still sets `tp_jolt`'s ISA options as PUBLIC, without `-mbmi2` or `-mno-fma`. Row by row in §5.10.4(b) |
| schemac, reflect, ECS, records, assets, physics, PCG, script | schemac, reflect and ECS (with `engine/ecs/SPIKES.md`) in the working tree; `engine/script` started in the working tree; the rest not started | Same build: `reflect_tests` 53/53, `ecs_tests` 90/90, `schemac_tests` 51/53. The two failures are diagnostic-text mismatches in an in-flight edit: the tests expect "`@editor(order=)` needs a 32-bit integer", and the code still says "needs an integer". WP-0.6–0.10 |
| RHI, render graph, Slang, rendertest | RHI (Vulkan and Null) and `apps/samples/rhi_triangle` in the working tree; render graph, `helios-shaderc` and `helios-rendertest` in the working tree (WP-0.12) | `engine/render` holds render graph v0 (compile, barriers, execute, text and Graphviz dumps), `.hsr` shader reflection, a forward v0 pipeline and ꟻLIP. `tools/rendertest` renders six scenes twice against Null traces and `vulkan-llvmpipe` goldens. Not rebuilt in the 2026-09-25 headless check; WP-0.11–0.12 |
| HTP transport, cell, gateway | `engine/net` (with benchmarks and fuzz targets) started in the working tree; cell and gateway not started | WP-0.13–0.14 |
| **`services/` Go backend** | **In the working tree, built ahead of the round order**; conformance rework open (WP-0.15r) | `helios-backend` runs identity, session and connect tokens, and the orchestrator (PG-anchored leadership, lease generations, ID blocks, local process supervision) on embedded PostgreSQL 18, NATS and miniredis. `pkg/` holds authn, connecttoken, idgen, keyring, ratelimit, rpc and testkit. On 2026-09-25, `go build`, `go vet` and `go test ./...` pass: 16 packages with tests, Go 1.27.1 through `go.mod`. It was first written to draft v1's lease and node-ID design and has since been largely reworked. The remaining deltas (schema names, PII encryption, `region_lease`, failure domains) are WP-0.15r (§5.10.4). Not yet built: the ledger schema (WP-0.15) and patching (WP-0.16) |
| Launcher, client, editor; project `schemas/`; `content/` | Not started | `schemas/` holds only schemac samples and tests (`sample/`, `test/`), not the *Cinder Reach* package. `content/` does not exist (WP-0.17–0.20) |
| Scorecard, nightly, GPU runner | Partial | `nightly.yml` exists (above). No `scorecard.jsonc`, perf history, `validate.ps1` or `win-gpu` runner (WP-0.3, 0.4; the runner only under §5.4a) |
| Plan conformance (§5.10) | Rules defined; tooling not started | CONF lint and `map.jsonc` in WP-0.2. Checked by hand on 2026-09-25 under D7, including the working tree: §5.10.4 (a) for the backend and (b) for ISA and the gate |
| Docs pipeline, SDK, templates | Not started | DX track from WP-1.24 (§2.7) |
| Funding | No envelope approved | F0 at the Phase 0 exit (§4.3.6); the WP-0.4 purchase list prices H1/H2 before then |

### 8.2 Next 12 work packages, in execution order

| # | Round | WP | Why now |
|---|---|---|---|
| 1 | 1 | **WP-0.1** CI matrix | AAA-PLT-1 gates Phase 0; every WP needs the MSVC and clang-cl signal; the `main` ruleset and merge queue (§5.2a) |
| 2 | 1 | **WP-0.2** Layering, licence, IP and conformance lints (without ISA levels) | Must exist before modules multiply (reconciliation #3). Its layering and lint code is in the working tree. CONF-01…12 check WP-0.15r, WP-0.2r and WP-0.5r (§5.10) |
| 3 | 1 | **WP-0.15r** Conformance rework of `services/`, then WP-0.15's remaining scope | Rework WPs rank right after P0 fixes (§5.10.2 D4), and the in-tree backend must conform before anything builds on it. WP-0.15's ledger schema then completes and the backend merges. Types stay hand-written until 0.7's Go emitter lands |
| 4 | 1 | **WP-0.5** Core and math completion (without the gate) | CPUID, process spawn, async reads, `det::` extras and `Fixed64` are in the working tree and feed PCG, HXL and command flight |
| 5 | 2 | **WP-0.2r** ISA-level rework | A rework WP (D4) that must merge within two rounds. The in-tree per-file allowlist is the design ADR-011 retired, and WP-0.9's `avx2` kernel and WP-0.17's `base` launcher need image levels |
| 6 | 2 | **WP-0.7** schemac + reflect (interface first) | Head of the critical path |
| 7 | 2 | **WP-0.6** Spikes | Retires K2 and K3 before code depends on flecs or heaps; 0.6c decides where the gate lives in modular builds |
| 8 | 2 | **WP-0.11** RHI Vulkan + Null | Head of the rendering lane; AAA-REN-7 |
| 9 | 2 | **WP-0.13** HTP transport | Independent; NS-0 criteria |
| 10 | 3 | **WP-0.5r** CPU-gate rework | Needs 0.2r's levels and audit. It moves the Windows gate to the first TLS-callback slot before any `avx2` executable ships |
| 11 | 3 | **WP-0.3** Nightly, scorecard, `validate.ps1`, `tools/status/snapshot` | Phase exits cannot be measured without it; §8.1 becomes generated (D6) |
| 12 | 3 | **WP-0.12** Slang, graph, rendertest | Much of it is in the working tree. It completes RC-1 and unblocks 0.9, 0.18 and Phase 1 rendering |
