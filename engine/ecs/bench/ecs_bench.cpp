...
//                  [--spikes-only] [--seed=N] [--inframe-dontfragment] [--no-warmup-pass]
// Exit code: 0 = RT-01 PASS, 1 = FAIL.
...
RunResult runZone(u32 workers, u32 ticks, u32 warmup, u64 seed, bool inFrameDontFragment, bool verbose) {
    ...
    desc.relations.inFrameDontFragment = inFrameDontFragment;
    {
        ...
        r.burstCold = zone.structuralBurst(0);
        std::vector<bench::BurstResult> warm;
        for (u32 round = 1; round <= kWarmBursts; ++round) warm.push_back(zone.structuralBurst(round));
        ...
        for (u32 round = kWarmBursts + 1; round <= 2 * kWarmBursts; ++round) raw.push_back(zone.rawFlecsBurst(round));
...
int main(int argc, char** argv) {
    ...
    if (!spikesOnly) {
        ...
        f64 iterate = 1e30, structural = 0, structuralMax = 0, structuralRaw = 0, ecsMb = 0;
        u32 tables = 0;
