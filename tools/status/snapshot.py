...
def plan_rev(root: Path) -> int:
    ...
    m = re.match(r"\s*(\d+)", path.read_text(encoding="utf-8"))
    if not m:
...
def readme_rev(root: Path, module: str) -> tuple[bool, int | None]:
    ...
    if not readme.is_file():
        return False, None
    m = re.search(r"^Plan-Rev: (\d+)", readme.read_text(encoding="utf-8"), re.M)
...
def row_line(status: str) -> str | None:
    return next((line for line in status.splitlines() if line.startswith(ROW)), None)
...
def inventory_cell(root: Path) -> str:
    ...
    groups = []
    for top in ("engine", "apps", "services", "tools"):
...
def write_inventory(root: Path) -> bool:
    ...
    new = "| " + " | ".join(cells[:-1] + [inventory_cell(root)]) + " |"
    if new == line:
        return False
...
def report(root: Path, inventories: list[dict]) -> str:
    ...
    out += ["", "### WP branches (local git refs)", ""]
    try:
