...
| Vendored patches | `vendor_patches.cmake` | a `third_party/<dep>/patches/` file not named `NNNN-<slug>.patch`, or not named in `third_party/MANIFEST.md`; a hunk whose post-image (context plus added lines) is not in the committed file, in hunk order; a patched file that is missing, or that the patch deletes but still exists. Line endings are normalized, so CRLF checkouts pass; no git or network | CLAUDE.md (vendored code), `third_party/MANIFEST.md` "Patches", K10 |
